...
export default class Resources extends React.Component {
  ...
  render() {
    ...
    const items = resourcesData.map((item, index) =>
      <MDBAnimation key={index} type={index % 2 === 0 ? 'flipInX' : 'zoomIn'} duration="2s" delay="0.3s">
        <div className="p-2">
          <MDBBtn onClick={this.toggleCollapse.bind(this, item.class)} className="w-100 d-flex justify-content-between align-items-center gradient-bg-primary m-0 p-2">
            <p className="m-0" style={{ fontSize: '1.375rem' }}>Class {item.class}</p>
            <MDBIcon style={{ fontSize: '1.375rem' }} className={'accordion' + (collapseID === item.class ? ' active-accordion' : '')}
              icon={'angle-down'}
            />
          </MDBBtn>
      ...
      <React.Fragment>
        <MDBContainer fluid className="pl-0 pr-0">
          <MDBAnimation type="" duration="">
            <MDBView src='https://ik.imagekit.io/excellenceclasses/patrick-tomasso-Oaqk7qqNh_c-unsplash_sj0sFhbbc.jpg' className="h-70vh">
