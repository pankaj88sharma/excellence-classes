      ...
      <MDBAnimation key={index} type={index % 2 === 0 ? 'zoomIn' : 'zoomIn'} duration="1s" delay="0.3s">
        <div className="">
          <MDBBtn onClick={this.toggleCollapse.bind(this, item.name)} className="w-100 d-flex justify-content-between align-items-center gradient-bg-primary m-0 p-2">
            <p className="m-0" style={{ fontSize: '1.1rem', fontWeight: '500' }}>{item.name}</p>
            <MDBIcon style={{ fontSize: '1.1rem' }} className={'accordion' + (collapseID === item.name ? ' active-accordion' : '')}
              icon={'angle-down'}
            />
          </MDBBtn>
          <MDBCollapse id={item.name} isOpen={collapseID} className="">
            {
              ...
              <MDBAnimation key={index} type={index % 2 === 0 ? 'slideInLeft' : 'slideInLeft'} duration="1s">
                <div className="pt-2 pb-2 pl-1">
                  <a className="text-white resource-link" href={item.link} style={{ textDecoration: 'underline', fontWeight: '300', fontSize: '1.1rem' }} target="_blank" rel="noopener noreferrer"> <MDBIcon className="mr-2" icon="link" />Chapter {item.id} {item.title}</a>
                </div>
                </MDBAnimation>
              )
            }
